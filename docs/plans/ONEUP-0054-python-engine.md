...
5. Apply the same array by hand at the broken-pipe scenario — the one that
   ...
   → **verify:** `grep -c 'bash "$ENGINE"' tests/run-tests.sh` returns 0 on
   `main`. **Not** "the scenario still passes": with the variable unset the
   array expands to `bash $ENGINE`, so this scenario behaves identically
   whether or not the step was performed, and its checks pass either way.
...
6. **Prove the indirection is not inert.** This is the step that makes stage 1
   ...
   suite, and see it go red. Unset it and see it go green again. **Then run the
   broken-pipe scenario against the same stub and see that scenario's own
   checks fail.** Whole-suite redness is evidence about `run_engine` alone —
...
7. Leave the readers that treat `$ENGINE` as a **file** alone — the
   ...
   → **verify:** none of those three readers appears in `git diff` on `main`.
...
9. Commit on `main`, then merge `main` into `v2` (§1.2, §2). Step 1 having
   ...
   pre-merge content, and `./local-CI.sh` is green on `v2`.
...
**Stage 1 is done** when `main`'s §4.4 matches `v2`'s, no `bash "$ENGINE"`
invocation remains in `tests/run-tests.sh`, `./local-CI.sh` is green on `main`
with `ONEUP_ENGINE_CMD` unset and red with it pointed at a failing stub — the
broken-pipe scenario included — and green again on `v2` after the merge.
`local-CI.sh` runs `bash tests/run-tests.sh` without scrubbing the environment,
so the override does reach the suite. Nothing about `main`'s behaviour changes.
