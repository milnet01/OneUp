...
conversion easiest to over-do, and it applies to four of the five families — every one but
`@@REMEDY@@`, whose payloads are already codes, so it has no sentence to retain. The engine
...
`"  Flatpak apps: couldn't check — $unreadable"`; `end_step`'s `detail` is stored in
`DETAIL[$key]` and printed by the end-of-run summary block; and `$REBOOT_REASON` feeds both
`marker REBOOT` and the summary's own `echo "  ! A REBOOT is recommended — $REBOOT_REASON."`
— so the engine keeps composing that phrase for the terminal while emitting components in
the marker, and converting `reboot_reason_from_log` in place would print
*"A REBOOT is recommended — kernel-new graphics-driver-nvidia"* to the one reader it has.
§10 keeps that terminal output
...
`@@REMEDY@@`'s two actions are **already codes**, so no new code is allocated for them;
they are in fate 2 for the naming discipline, not the conversion. Their call sites still
change with every other (§4.2): `skip-repo` must pass its alias as a **separate** argument,
because left pre-joined the guard rewrites its `|` to `/` and the window's branch, which
requires a second field, arms no button. A converted call site passes a code
...
- **INV-1** Every code matches `^[a-z0-9-]+$`, and every **code field** holds codes and
  ...
  vocabularies — every element a member of the component set, or the whole field a single
  standalone code — and not by shape**, because ordinary lowercase English passes the shape check
  ...
  *Test:* `tests/run-tests.sh` asserts the shape of **the code field only** — counting the
  ...
  and every space-separated element of `@@REBOOT@@`'s reason, each of which is asserted to
  be a member of §4.1's component set unless the whole field is a single standalone code —
  never merely code-shaped, and never classified by how many elements it has, since a
  kernel-only transaction carries exactly one component (`kernel-new`). The argument fields beside them
...
| The retained Bash engine is run against a converted window | Prose where the window expects a code, so `ONEUP-0108` INV-1's fallback sentence — except `end_step cache ok`, the one call with no `detail`, which sends an **empty** field rather than prose | Deliberate and known: the fallback is frozen at the switch-over. `oneup-2.0.md` §4 requires it in the release notes, and §8 carries that. Wording the empty field is `ONEUP-0108` INV-1's, not this item's |
...
- **`docs/reference/marker-protocol.md`** — §3's field table; §4.1, whose four-field guard
  ...
  which currently reserve this work for `HINT` and `REMEDY` alone (§3.1) and whose
  *"never as the raw token"* is scoped to the sites with room for a sentence, since
  `docs/specs/ONEUP-0108-window-wording.md` §4.3's **short** form is the bare code, and the
  three sites it names — a `@@STEP_END@@` badge, the progress-bar caption and each element
  of a `@@REBOOT@@` join — are all this item's. That document's §8 assigns the edit here,
  and its §9 already reads §5.2 this way without anything having amended it; **§1.1 and §6's
...
- **`docs/standards/testing.md`** — §5's invariant 2 reads *"A failed step is recorded, emits
  ...
  it says what the marker carries. **§1's suite table loses its `tests/differential-test.sh`
  row with the file** (§7) — the row exists only if ONEUP-0054 has landed it, and leaving it
  behind points that table at a file this commit deletes.
...
- **`docs/design/oneup-2.0.md`** — **§4** said this item "converts the `@@HINT@@` and
  ...
  *cites* §4. §4 now carries the wide form and points at §3.1. **§7's G10 row** carried the
  ...
  conversion" — and §3.1 cites G10 by name, so it is widened in step. **§3 item 1** is the
  third and the one that mattered most: it is that document's protocol-freeze clause, so
  the narrow wording made the four changes outside the pair — one retirement and three
  conversions — read as breaches with no exception covering them. Widened with the other two.
...
| --- | --- | --- | --- |
| 6 | 2026-08-19 | 3 lanes, cold; second and final loop of this run, so the spec cap of 2 binds; Q1 1 · Q2 3 · Q3 0 · Q4 0, 4 verified, 3 dismissed, 1 filed against a neighbour | **A calm cap — not one of the four landed on text this run wrote.** Loop 5's three additions (the `@@REMEDY@@` fate-2 paragraph, INV-1's membership rule, §6's last row) drew no finding at all; the document simply held more defects than the cap held loops, which is the case where shipping is right. **The Q1 is the run's best and only one lane found it.** §4.1's *"The engine keeps its English"* said it applies to **three** of the five families, and it is four: `$REBOOT_REASON` feeds both `marker REBOOT` and the summary's own `echo "  ! A REBOOT is recommended — $REBOOT_REASON."` (`update_system.sh` at the `marker REBOOT` line and in the summary block). REBOOT is the family §4.1 most actively tells the implementer to change — *"from a phrase to a list"* — so it was the one excluded from the paragraph that exists to stop the change gutting the terminal: converting `reboot_reason_from_log` in place prints *"A REBOOT is recommended — kernel-new graphics-driver-nvidia"* to the one reader §10 keeps English for. Loop 2 found this obligation for the other three families and missed the fourth; the loop-2 ledger records the same three. **All three Q2s are the same defect in the same place** — §8 is the commit-time doc-edit list and three edits were missing from it, each found by two lanes independently. §8 widened `oneup-2.0.md` §4 and §7's G10 and left **§3 item 1**, that document's protocol-freeze clause, still naming the `@@HINT@@`/`@@REMEDY@@` pair alone — so the four changes outside the pair read as breaches with no exception covering them; widened here with the other two. §7 requires the landing commit to strip `tests/differential-test.sh` from `testing.md` §1's suite table and §8's `testing.md` bullet named only §5's invariant 2, while §8 closes by enumerating what it *deliberately* omits — so the list read complete and the commit would have left a dangling pointer `tests/docs-check.py` fails on. And `marker-protocol.md` §5.2's *"never as the raw token"* is unscoped while `ONEUP-0108` §4.3's **short** form is the bare code at three sites that are all this item's — `ONEUP-0108` §8 assigns marker-protocol's substantive edits here, and its §9 already reads §5.2 as scoped without anything having amended it. **Three dismissed.** One unverified on a false premise — a lane read §4.2's omitted-trailing-field rule as leaving a zero-field marker undefined, and no such call site exists: all 23 markers carry a payload. Two true but immaterial: the `installed` row's *"packages were installed"* does not describe the two call sites that reach that badge (both say *updated*), but the `Args` column decides every site carrying a number; and §4.2's *"reserved three questions for this spec"* is contradicted by §5.2 giving question 2 to the window, where the routing is stated in three other places and no lane raised it. **Filed, not fixed:** `oneup-2.0.md`'s G1/G2 passage attributes the payload conversion to ONEUP-0032, stale from before the 2026-08-03 split — a neighbouring document's defect, left for its own edit. The packet windowed every citation and turned up two of the four findings while doing it; its one abridged window, `ONEUP-0108` §4.3's short-form definition, cost all three lanes an open question. |
| 5 | 2026-08-19 | 2 lanes, first loop of a fresh run after the ONEUP-0101 split took §4.3 away; Q1 1 · Q2 2 · Q3 0 · Q4 0, 3 verified, 1 dismissed (no severity scale under the four-question gate, so nothing here for §7's tally check to balance; ONEUP-0100) | **Both lanes independently led with the same defect, and it is the one that would have turned the suite red on a correct run.** INV-1 said `@@REBOOT@@`'s reason is checked *"components where it holds several elements, the standalone set where it holds one"* — selection by element **count**, which §4.1 of this same document rules out four lines from the vocabulary table: the rule *"turns on which vocabulary the field matches … not on how many elements it has"*. A kernel-only transaction is a **one-element components** field (`reboot_reason_from_log` appends *"a new kernel"* alone and agrees `verb="was"` for exactly that case), so an implementer following INV-1 asserts membership of the standalone set and the suite fails on the commonest reboot there is — on the very scenario INV-1's own test list tells them to add. The likely repair under pressure is to weaken the assertion back to `^[a-z0-9-]+$`, which is the vacuous shape check §4.1 proved lets half-converted prose through, so the defect **converts INV-1 into the thing it was written to prevent**. Fixed by membership of the union plus homogeneity — every element a component, or the whole field one standalone code — in both the invariant and its test clause, which restated the same wrong key in different words. `ONEUP-0108` §4.4 already classifies this way and tests the standalone set first, so the fix aligns the engine-side invariant with the window-side rule rather than inventing one. **The second was found by one lane and by the packet build independently, and executing it settled it.** §4.1 said `@@REMEDY@@`'s two actions *"need no call-site change"*, while §4.2 says the emitter signature change *"touches **every** `marker` call site"* — and the engine really carries `marker REMEDY "skip-repo\|$alias"`, a pre-joined two-field payload. Run through the mandated variadic emitter with INV-2's per-argument guard it becomes `@@REMEDY@@\|skip-repo/packman`, and `handle_marker`'s arm requires `parts[0] == "skip-repo"` with at least two fields, so the *Skip this source* button silently stops arming with no assertion failing — INV-1 only shape-checks field 1. Both payloads were put through the emitter and the window's branch rather than reasoned about: pre-joined arms nothing, separated arms correctly. §4.1's own sentence three lines above already said it (*"if it still builds a string for the marker, it is not done"*), which is what made the false clause survivable. **The Q1 came from a lane's open question it could not settle from the packet.** §6's last row promised that the retained Bash engine against a converted window yields `ONEUP-0108` INV-1's fallback throughout; `end_step cache ok` is the **one** call of twenty-one with no `detail`, so it sends an *empty* field rather than prose, and INV-1 there requires text *"non-empty"* containing every unknown code it was fed — unsatisfiable when there is no code. The row is narrowed to say so and the wording obligation named as `ONEUP-0108`'s; **filed, not fixed**, because adding an empty-field case to that document's INV-1 is a contract addition with its own gate. **One dismissed as out of scope:** a lane asked whether the emitter's middle-`None` raise collides with `security.md` §6.3's *"never add a code path that kills the engine mid-run"*. It does not — that sentence sits under *A run must survive the GUI going away* and is scoped to signal- and pipe-initiated death, with *"closing to the tray is not a quit"* immediately after. **Three lane open questions resolved clean and are recorded so a later loop does not re-ask**: `@@PROGRESS@@`'s `phase` is fate 3 correctly (the window composes its own verb from it); `@@REPO_SKIPPED@@`'s `reason` is fate 3 correctly (the branch appends only the alias); and zypper's `Skipping repository '…'` really does print the repository **name**, re-measured against a `--root` install carrying a space-free alias and a spaced name, which returned `Skipping repository 'Broken Test Repository (With Spaces)'` — the alias never appeared, so the one-name-per-field wire shape holds. **The run stopped on its `--max-loops 1` argument, not on the document's cap of 2 — a second cold loop is available and unspent.** Nothing this loop landed on text this run wrote, since this was its first; the document left the loop at **740 lines**, up from 735. |
