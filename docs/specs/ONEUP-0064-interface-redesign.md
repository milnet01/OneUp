...
- **INV-7** Every object name styled in `_QSS` has a counterpart rule in `_HC_QSS`, except
  ...
  *Test:* `tests/gui-smoke.py` extracts the set of object-name selectors from each
  ...
  templates carry `#rrggbb` literals in *declarations*, so a naive `#(\w+)` scan collects
  `d6412a`, `e04a32`, `e0553f`, `ef6a55`, `f47c68` and `ffffff` as base-only "names" and the
  test is red on its first run — the failure this invariant's own exception list exists to
  avoid. (Both sheets also carry class-only rules, and not the same ones, so the extractor
