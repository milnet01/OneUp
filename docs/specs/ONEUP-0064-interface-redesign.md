...
read. All seven are given verbatim so §10's hand-off to `ONEUP-0032` covers them rather
than just the one:
...
`rgba(231,76,60,0.05)` wash — a red edge over an all-but-transparent ground, which is the
half of the construction *Stop* wants. The banner's own ink is **not** red
(`QLabel#BannerText` takes `$header`); putting the danger colour on the label as well as the
border is *Stop*'s addition, not something it copies. `#RestartBtn` is the opposite:
...
`:disabled` and `:focus` in **both**, leaving it unstyled rather than
mis-styled. (`:checked` is not in that list: `stop_btn` is never `setCheckable(True)`, so
`#GhostBtn:checked` has never matched it and `#StopBtn` needs no such rule.) That includes the high-contrast overlay, which is the one appearance mode that
...
**The handler is inert whenever the switch is disabled.** `set_controls_enabled` disables
each row's switch for the length of a run but leaves the row frame enabled, and a disabled
`ToggleSwitch` still emits `toggled` from `setChecked()` or `toggle()` — only `click()` is
ignored. So the row-level handler tests `switch.isEnabled()` before acting; without that a
user could re-select tasks mid-run by clicking a row's text, which would be a second
behaviour change §4.2 says this item does not make.

**The disclosure arrow grows to 24×24**, which §2.2 establishes is an ergonomics fix rather
than a conformance one, and **gains a `:hover` rule in both sheets** — it has none today:
`QToolButton#Disclose` carries one rule, `background: transparent; border: none;
padding: 0px;`, with no `:hover` variant in either sheet. **There is no single "hover
...
| each row's **body** | out of the **checked set**, not out of the change | newly clickable, and a target — but not a *widget*: it is the row less its four excluded children, so every object a test could reach for (`TaskRow`, its `#RowCard`) spans `card`'s width and contains the 56×30 switch, and the assertion could never go red. Measured at a 6 pt application font it is 516×56. INV-4 exercises it with a posted click instead, which is an assertion that can fail |
...
- **INV-2** Every focusable widget still reports a non-empty accessible name **or visible
  ...
  *Test:* the existing `tests/gui-smoke.py` name sweep — ONEUP-0028's guarantee, **re-run
  unchanged** against the redesigned tree. It already opens `SettingsDialog`,
  `RepoManagerDialog` and `RollbackDialog` and sweeps each, so the two controls §4.1 moves
  stay covered with no extension. **Nothing here narrows it:** INV-1 and INV-5 scope to the
  window, its banners and `SettingsDialog` (§10); INV-2 keeps the wider reach it has. The "or visible
...
- **INV-4** Clicking a task row's body toggles that row's switch exactly once; clicking the
  ...
  *Test:* `tests/gui-smoke.py` counts each switch's `toggled` emissions rather than
  ...
  are asserted separately. **A fifth case covers the run:** after `set_controls_enabled(False)`
  a body click asserts 0 emissions, because a disabled `ToggleSwitch` still emits from
  `setChecked()` and `toggle()` and only `click()` is ignored.
...
- **INV-5** Every **pointer target** measures at least 24×24, width and height, at 6 pt —
  ...
  (focusable, not targets). Each task row's newly clickable body is a target too, but it is
  a region rather than a widget and its table row says why nothing here can measure it —
  INV-4 covers it instead.
...
- **INV-7** Every object name styled in `_QSS` has a counterpart rule in `_HC_QSS`, except
  ...
  *Test:* `tests/gui-smoke.py` extracts the set of object-name selectors from each
  template's text — **from selector position only** — and asserts the base sheet's set, less
  the exception list, is a subset of the overlay's. **Position is the whole of it:** both
  templates carry `#rrggbb` literals in *declarations*, so a naive `#(\w+)` scan over the raw
  text collects colour values as names — twelve of them base-only, reds and blues both,
  because `_HC_QSS` carries no hex literal at all — and the test is red on its first run,
  the failure this invariant's own exception list exists to avoid. (Both sheets also carry class-only rules, and not the same ones, so the extractor
  ignores every selector without an object name.)
  **What a missing overlay rule actually does is mis-colour, not unstyle.** `build_theme`
  **appends** the overlay — `qss = _QSS.substitute(palette)`, then `qss += _HC_QSS
  .substitute(hc)` — rather than swapping it, so a name styled in the base sheet and absent
  from the overlay keeps every base property, drawn from `_DARK`/`_LIGHT`, over a
  high-contrast surface. That is worse than unstyled: it still looks deliberate. Breaks on `#StopBtn` if
  ...
  **The exception list is `#RowDetails` and `QScrollArea#DetailScroll`, and it is named
  ...
  safe for one reason, and it is the criterion a third would have to meet: **their base rule
  sets no colour** — both are transparent-background containers, so the append above has
  nothing to leak. They stay
...
- **Moving a control into `SettingsDialog` takes it out of a sweep that covered it.**
  *Repositories* and *Recenter* move, so every widget sweep in this spec opens
  `SettingsDialog` rather than stopping at the window — INV-1 and INV-5 each repeat that
  scope inside their own *Test:* clause, because a scope stated only here is a scope no test
  inherits, and INV-2's existing sweep already opens all three dialogs. The same move is why `docs/specs/ONEUP-0076-ringless-focus-cue.md`'s
...
| INV-2 | `tests/gui-smoke.py` | **existing, unchanged** — ONEUP-0028's accessible-name sweep, which already opens all three dialogs, re-run against the redesigned tree |
| INV-3 | `tests/gui-smoke.py` | **existing, unchanged** — the assertion that the built stylesheet carries no `font-size:` in `px` |
| INV-4 | `tests/gui-smoke.py` | **new** — reveals a row's badge, disclosure and detail panel, then clicks each plus the body and the switch, counting `toggled` emissions per case, and once more on the body with the switch disabled |
...
**One existing assertion is superseded, and it goes in the same commit as the move.**
`tests/gui-smoke.py` walks `settings_btn.nextInFocusChain()` looking for `repos_btn` and
`recenter_btn` and asserts *Repositories* is reached first. Once both are children of
`SettingsDialog`, a walk rooted in the window can never reach them — INV-1 says so itself —
so the collected chain is empty and the assertion is red. It is **removed**, not repaired:
INV-1's dialog walk carries the same guarantee, because §4.1's grouping table puts
*Repositories* above *Recenter* under *This machine* and INV-1 asserts the dialog's chain
matches its visual order. Nothing else in the suite asserts on either control's position.
...
| --- | --- | --- | --- |
| 6 | 2026-08-13 | 2 lanes, third loop of this run — the default cap, so the run stops here: Q1 3 · Q2 1 · Q3 1 — 5 verified, 0 dismissed, of which **two were this run's own collateral** | **Stopped at the cap with findings still arriving, and two of the five were mine — which is the honest reading of where this document is.** Both lanes led with the same one, and it was loop 5's own fix: that fix said a naive `#(\w+)` scan over the templates collects six colour literals as base-only names, naming the red family. The real figure is **twelve**, reds and blues both — `2560c8`, `3a7cf0`, `3d90ec`, `4aa3ff`, `5cb0ff`, `6fb6ff` were missed, and `_HC_QSS` carries no hex literal at all, so every one of them is base-only. An implementer taking the six as complete would filter them instead of extracting by selector position and still be red on `4aa3ff` — the exact failure the paragraph exists to prevent. The enumeration is gone rather than corrected: a census in prose goes stale, and the shape is what the implementer needs. **The second was loop 4's, and it could have deleted working coverage.** Loop 4 narrowed INV-2 to "extended to open `SettingsDialog`" and added "the other two dialogs stay out". Both halves are false about the suite as it stands: `tests/gui-smoke.py` already builds and sweeps `SettingsDialog`, `RepoManagerDialog` **and** `RollbackDialog` for accessible names, so no extension was ever needed — and read as an instruction, "stay out" tells an implementer to remove two live ONEUP-0028 checks that §4.2 calls a regression bar. INV-2 is now "re-run unchanged", with the reason it is not narrowed stated where §6 restates the scope. **The gap the lanes found on their own is an assertion this item breaks and nothing said so.** `tests/gui-smoke.py` walks `settings_btn.nextInFocusChain()` for `repos_btn` and `recenter_btn` and asserts *Repositories* comes first; once both are children of `SettingsDialog` that walk can never reach them, the collected chain is empty and the assertion goes red — leaving the implementer to invent its disposition mid-build, the worst answer being to keep both controls in the window's chain to keep it green and so defeat the move. §7 now retires it explicitly and says which invariant carries the guarantee instead. Two smaller Q1/Q2s: §4.1's string table is introduced with "All six are given verbatim" over a seven-row table that §10 correctly hands to `ONEUP-0032` as seven — the seventh, the banner fallback, is the one that would have shipped unwrapped; and the rename paragraph listed `:checked` among the pseudo-states *Stop* falls out of, where `stop_btn` is never `setCheckable(True)`, so `#GhostBtn:checked` has never matched it and `#StopBtn` needs no such rule. **Still open and unchanged: the two `:hover` colours** (`QToolButton#Disclose`'s ink, `#StopBtn`'s border and label), stated in §4.1 as the one decision this document leaves to the user. **What the cap means here.** Three loops produced 9, 6 and 5 verified findings, and the document went 556 → 626 lines across them; loops 1–3 of the earlier run had already found 30, 28 and 23. A document still yielding build-changing findings on its sixth cold read, while growing, is the size signal rather than a quality one — at 626 lines it is the third-longest spec in the tree. The next session should decide whether to split §4.1 (the layout narrative, the string table and the target-size table are three contracts in one section) before spending a seventh. |  2 lanes, second loop of this run: Q1 2 · Q3 3 · Q4 1 — 6 verified, 0 dismissed, and none of the six landed on a passage loop 4 edited | **Every finding was pre-existing, so loop 4's fixes held and this loop is what the run was for.** Both lanes independently led with the same Q1, and it inverts the mechanism INV-7 is built on: INV-7's *Test:* said a name styled in one sheet and not the other "is not mis-coloured in high contrast — it is **unstyled** there", but `build_theme` **appends** the overlay (`qss = _QSS.substitute(palette)`, then `qss += _HC_QSS.substitute(hc)`), and `updater.py`'s own comment at `_HC_QSS` says so. A base-only rule therefore survives into high contrast carrying `_DARK`/`_LIGHT` colours over a high-contrast surface — worse than unstyled, because it still looks deliberate. The exception list rested on the same wrong model: "nothing to restate in high contrast" was the stated criterion for exempting a third name, where the real one is **the base rule sets no colour**, which is why those two containers are safe and a coloured rule would not be. Both are corrected, and the invariant now says what the append does. **The second Q1 is an exemplar that is half wrong in the direction that matters.** §4.1 described `#RebootBanner` as "a red edge and red-tinted ink"; the banner's label is `QLabel#BannerText`, which takes `$header` — there is no red ink anywhere in it, so an implementer copying the named construction gets the border and not the label, against a sentence that twice requires both. **A test clause that could not have gone red, and one that would have been red on arrival.** INV-5 listed "each row's body" as a measured 24×24 target, but a body is a region rather than a widget: every object a test could reach for spans `card`'s width and contains the 56×30 switch — measured at 6 pt it is 516×56 — so the assertion was unfalsifiable; it moves out of the checked set to INV-4, which exercises it with a posted click that can fail. And INV-7's extractor was specified as reading object names "from each template's text", where both templates carry `#rrggbb` literals in declarations: a naive scan collects `d6412a`, `e04a32`, `e0553f`, `ef6a55`, `f47c68` and `ffffff` as base-only names — reproduced here — making the test red on its first run, which is the exact failure its exception list exists to prevent. Selector position is now part of the contract. **The last gap is a behaviour nobody had ruled out.** §4.1 makes a row's body clickable but never said what it does mid-run: `set_controls_enabled` disables each switch and not the row frame, and a disabled `ToggleSwitch` still emits `toggled` from `setChecked()` and `toggle()` — measured, only `click()` is ignored — so a body handler would have let a user re-select tasks during a run, a second behaviour change §4.2 says this item does not make. The handler now tests `switch.isEnabled()` and INV-4 gains a fifth case. **Still open, and now stated in §4.1 rather than only here:** the `:hover` colours for `QToolButton#Disclose` and `#StopBtn`. Loop 4 surfaced the second; this loop found the first independently, which is what made it a class rather than an oversight — §4.1 pins the property for each (ink for the arrow, border and label for Stop) and leaves the values to the user, because `#GhostBtn`'s `#4aa3ff` would turn the danger control blue and a rest-coloured hover is no hover at all. The document left this loop at **614 lines**, up from 578. |
