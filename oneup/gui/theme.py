...
# The six palettes ONEUP-0027 adds, beside the two that already shipped. Each was
# generated by tinting `midnight`/`daylight`'s SURFACES toward a hue and then
# correcting every foreground the check failed — done ONCE, at authoring time,
# with the output frozen here as literals. §9 rejects deriving a palette from a
# hue at RUN time, and for a reason this does not touch: a generated palette has
# no way to satisfy a 3:1 boundary except by luck, so it would need this same
# check anyway. What makes freezing them safe is that INV-2 re-measures every
...
_CARBON = dict(
    ...
    infotint2='#22d3ee', dangertint='#e74c3c'
)
...
class Theme:
    """`(id, label, base, palette)` — ONEUP-0027 §4.1.

    `base` is what decides which of the two high-contrast overlays is appended
    and what Follow system matches against; nothing else reads it. The `id` is
    what gets stored and never changes; the `label` is what Settings shows and
    is translatable (ONEUP-0032). A named theme is ONE palette, not a light/dark
    pair — choosing one means choosing not to follow the desktop, which is what
    Follow system is for.
    """

    __slots__ = ("base", "id", "label", "palette")

    def __init__(self, id_: str, label: str, base: str, palette: dict):
        self.id, self.label, self.base, self.palette = id_, label, base, palette
...
SYSTEM = "system"          # not a theme but a MODE, and the initial default

THEMES: list[Theme] = [
    Theme("midnight", "Midnight", "dark", _DARK),
    Theme("carbon", "Carbon", "dark", _CARBON),
    Theme("forest", "Forest", "dark", _FOREST),
    Theme("plum", "Plum", "dark", _PLUM),
    Theme("daylight", "Daylight", "light", _LIGHT),
    Theme("paper", "Paper", "light", _PAPER),
    Theme("sky", "Sky", "light", _SKY),
    Theme("sand", "Sand", "light", _SAND),
]
BY_ID = {th.id: th for th in THEMES}

# The reference set is `midnight`'s key set — not a list written down here, which
# would rot the first time a token was added (§4.1). The high-contrast overlay is
# deliberately NOT part of it: §7 keeps it an overlay rather than a theme.
REFERENCE_KEYS = frozenset(_DARK)


def resolve(theme_id: str) -> Theme | None:
    """The theme an id names, or None for Follow system and anything unknown.

    Qt returns whatever is stored, valid or not — a `theme` key holding
    `Forrest`, or an id from a later version, comes back verbatim — so the
    default argument is NOT the fallback and the lookup has to be explicit.
    """
    return BY_ID.get(theme_id)
...
def focus_report(theme, high_contrast: bool = False) -> list[dict]:
    ...
    if isinstance(theme, bool):
        theme = BY_ID["midnight" if theme else "daylight"]
    rows: list[dict] = []
...
def build_theme(theme, scale: float = 1.0, high_contrast: bool = False) -> str:
    """The stylesheet for one theme.

    Takes the THEME, not a `dark` flag: with eight palettes chosen by id there is
    nothing for a boolean to select. `True`/`False` are still accepted and mean
    `midnight`/`daylight`, because the suite and the focus report ask for a base
    palette by name and a second entry point would be a second thing to keep
    right.
    """
    if isinstance(theme, bool):
        theme = BY_ID["midnight" if theme else "daylight"]
    ...
    if high_contrast:
        # The overlay is picked from the theme's `base` — §4.1's fourth field,
...
# Set when a theme could not be applied, so the picker can say so rather than
# leaving a control that silently does nothing (§4.5, INV-13). Read and cleared
# by the picker; the console line is not a user-visible report.
last_theme_error: str = ""


def system_theme(app: QApplication):
    """What Follow system resolves to right now."""
    return BY_ID["midnight" if current_is_dark(app) else "daylight"]


def chosen_theme(app: QApplication):
    """The theme the stored preference names, and whether it was recognised.

    An unrecognised id — a downgrade, a hand-edited config, a theme from a later
    version — starts in Follow system and the STORED VALUE IS LEFT ALONE, so a
    user who downgrades, starts once and upgrades again gets their theme back.
    """
    stored = QSettings("OneUp", "OneUp").value("theme", SYSTEM)
    if stored == SYSTEM:
        return system_theme(app), True
    theme = resolve(str(stored))
    if theme is None:
        return system_theme(app), False
    return theme, True


def apply_app_theme(app: QApplication):
    """Install the stylesheet for the chosen theme plus the user's accessibility
    preferences. The single place the QSS is applied — startup, a light/dark
    switch, the picker and the two Settings controls all route through here, so a
    change takes effect live with no restart and no window rebuild."""
    global _CURRENT_PALETTE, last_theme_error
    s = QSettings("OneUp", "OneUp")
    theme, _known = chosen_theme(app)
    scale = float(s.value("text_scale", 1.0, type=float))
    high_contrast = bool(s.value("high_contrast", False, type=bool))
    try:
        qss = build_theme(theme, scale=scale, high_contrast=high_contrast)
        ...
        # ship the state ONEUP-0076 exists to end. Now that `build_theme` takes a
        # theme there is somewhere to fall back TO, which is what §4.5 asks for:
        # Follow system, the stored id left alone, and the user told.
        fallback = system_theme(app)
        last_theme_error = (
            f"{theme.label} could not be applied and was skipped: {exc}")
        print(f"OneUp: unusable theme — {exc}; falling back to Follow system")
        if fallback.id == theme.id:
            # The fallback IS what failed. Apply nothing and keep the sheet
            # already installed, which preserves the user's text size and
            # high-contrast choice — dropping those would take away the one
    ...
    # Setting the sheet re-polishes the widgets it styles, but a painter reading
    # `current_palette()` is not styled by it — nothing tells the switch its
    # colours moved. So repaint what the sheet cannot reach. This reaches the
    # switches through their parents; the tray icon has no parent and is rebuilt
    # by its own window (INV-8).
