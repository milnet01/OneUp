...
def main() -> int:
    ...
    wD.handle_line("@@CHECK@@|TOTAL|²")
    try:
        wD.on_finished(0, None)
    ...
    dlg_f = updater.RepoManagerDialog(None, repos)
    ...
    def _failed_apply(code, now_repos):
        d = updater.RepoManagerDialog(None, repos)
        said = []
        _w, _r = updater.QMessageBox.warning, updater.read_repos
        updater.QMessageBox.warning = staticmethod(lambda _p, _t, text, *a, **k: said.append(text))
        updater.read_repos = lambda: now_repos
        try:
            d._on_applied(code, None)
        finally:
            updater.QMessageBox.warning, updater.read_repos = _w, _r
        return d, " ".join(said)
    half = [dict(r) for r in repos]
    half[0]["enabled"] = False                  # the disable landed ...
    half = [r for r in half if r["alias"] != "debug-dup"] + [dict(repos[2])]  # ... the remove didn't
    ...
    wr._snapshot = "42"
    started, said = [], []
    _warn, _info, _start = (updater.QMessageBox.warning, updater.QMessageBox.information,
                            updater.QProcess.start)
    updater.QMessageBox.warning = staticmethod(
        lambda _p, _t, text, *a, **k: (said.append(text), QMessageBox.Yes)[1])
    updater.QMessageBox.information = staticmethod(
        lambda _p, _t, text, *a, **k: said.append(text))
    updater.QProcess.start = lambda self, prog, args=None, *a, **k: started.append([prog, *(args or [])])
    try:
        wr.rollback()
        ...
        for code, want in ((126, "cancelled"), (3, "couldn't roll back"),
            ...
            said.clear()
            wr._on_rollback_finished(code)
            check(f"rollback exit {code} is explained", want in " ".join(said))
        said.clear()
        wr._on_rollback_finished(0)
        check("a rollback that is restarting says nothing", said == [])
    finally:
        (updater.QMessageBox.warning, updater.QMessageBox.information,
         updater.QProcess.start) = _warn, _info, _start
